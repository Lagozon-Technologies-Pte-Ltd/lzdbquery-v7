...
// Global variable to track the current WebSocket connection
...
//         taskWebSocket = new WebSocket(`ws://${window.location.host}/ws/task/${task_id}`);
//         console.log(`WebSocket created for task ${task_id}`, taskWebSocket);
...
//                 // Process updates progressively
//                 handleWebSocketUpdate(data);
                
//                 if (data.status === 'SUCCESS') {
//                     console.log("WebSocket received SUCCESS status");

//                     closeWebSocket();
//                     resolve(data.result);
//                 } else if (data.status === 'FAILURE') {

//                     console.log("WebSocket received FAILURE status");
...
//     console.log("handleWebSocketUpdate called with data:", data);
...
//             // Update LLM response as soon as it's available
//             if (data.llm_response) {
//                 console.log("Updating LLM response:", data.llm_response);

//                 updateLLMResponse(data.llm_response);
//             }
            
//             // Update SQL query when generated
//             if (data.query) {
//                 console.log("Updating SQL query:", data.query);

//                 sqlQueryContent.textContent = data.query || "";
//                 Prism.highlightElement(sqlQueryContent);
//             }
//             else {
//                 console.log("No query in PROGRESS update");
//             }
            
//             return; // Don't complete processing yet
//         }

//         // Final result processing (status === 'SUCCESS')
...
// // Helper function to update LLM response progressively
// function updateLLMResponse(llmResponse) {
//     console.log("updateLLMResponse called with:", llmResponse);

//     const chatMessages = document.getElementById("chat-messages");
//     console.log("chatMessages element:", chatMessages);

//     // Check if we already have an LLM message in progress
//     let llmMessageElement = document.getElementById("llm-progress-message");
//     console.log("Existing llmMessageElement:", llmMessageElement);

//     if (!llmMessageElement) {
//         // Create new message element
//         chatMessages.innerHTML += `
//             <div class="message ai-message" id="llm-progress-message">
//                 <div class="message-content">
//                     <strong>LLM Interpretation:</strong> <span id="llm-response-text">${llmResponse}</span>
//                 </div>
//             </div>
//         `;
//     } else {
//         // Update existing message
//         document.getElementById("llm-response-text").textContent = llmResponse;
//     }
    
//     chatMessages.scrollTop = chatMessages.scrollHeight;
//     console.log("LLM response updated successfully");
...
function processFinalResult(data) {
    ...
    try {
        ...
        sqlQueryContent.textContent = "Not available due to error";
        interpPromptContent.textContent = "";
        langPromptContent.textContent = "";
    }
}
...
//         // 2. Poll for results
//         let attempts = 0;
//         const maxAttempts = 30; // 30 attempts with 1s delay = 30s timeout
...
//             // Process the final result (same as your existing code)
//             const data = await resultResponse.json();
//             processFinalResult(data);
//         };
