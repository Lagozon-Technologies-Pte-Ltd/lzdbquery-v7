from fastapi import FastAPI, Form, HTTPException, Query, UploadFile, File,Depends,WebSocket, WebSocketDisconnect
...
#         asyncio.create_task(self._listen_for_updates(task_id))
...
#             except Exception as e:
#                 logger.error(f"Failed to send message to {task_id}: {str(e)}")
#                 self.disconnect(task_id)

#     def publish_update(self, task_id: str, message: dict):
#         """Publish update to Redis channel"""
#         r = redis.Redis(connection_pool=self.redis_pool)
#         r.publish(f"task_updates:{task_id}", json.dumps(message))

#     async def _listen_for_updates(self, task_id: str):
#         """Listen for updates from Redis and forward to WebSocket"""
#         r = redis.Redis(connection_pool=self.redis_pool)
#         pubsub = r.pubsub()
...
#                         await self.send_message(data, task_id)
#                     except json.JSONDecodeError as e:
#                         logger.error(f"Failed to decode message: {str(e)}")
#                 await asyncio.sleep(0.1)
#         except Exception as e:
#             logger.error(f"Error in listen_for_updates for {task_id}: {str(e)}")
#         finally:
#             pubsub.close()
...
#     await websocket.accept()
#     # await websocket.send_json({"status": "test", "message": "Hello from WebSocket!"})  # <-- Add this line
#     try:
#         while True:
...
# # Modify your Celery task to send updates via WebSocket
# @celery_app.task(bind=True)
# def process_query_task(self, task_data):
#     # Initialize response structure (same as before)
#     response_data = {
...
#     logger.info(f"Sending initial WebSocket update for task {task_id}")

#     try:
        
#          # Replace asyncio.run calls with:
#         manager.publish_update(task_id, {
#             "status": "processing",
...
#             # db = app.state.bq_client  # Add config if needed (e.g., credentials, project)
...
#         # Before returning, explicitly store the result
#         self.update_state(
#             state='SUCCESS',
#             meta=response_data,
